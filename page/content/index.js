...
async function setArtileList(){
  ...
  console.log(res);
  //1.3展示到指定的标签结构中
  const htmStr=res.data.results.map(item=>{
    return `<tr>
                <td>
  ...
  document.querySelector('.art-list').innerHTML=htmStr
...
async function setChannleList() {
  ...
  const htmStr = `<option value="" selected="">请选择文章频道</option>` + res.data.channels.map(item => {
    return `<option value="${item.id}">${item.name}</option>`
  }).join('')
  document.querySelector('.form-select').innerHTML = htmStr
}
setChannleList()
 ...
 * 3.3 点击上一页，做临界值判断，并切换页面参数并请求最新数据
*/
//3.2 点击下一页，做临界值判断，并切换页码参数并请求最新数据
